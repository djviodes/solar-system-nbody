    ...
    if let Some(name) = body_selected.as_deref() {
        if let Some(body) = bodies.iter().find(|b| b.name == name) {
            ...
            let distance_from_sun = format!(
                ...
                body.position.subtract(sun_position.unwrap_or(crate::vec3::Vec3::new(0.0, 0.0, 0.0))).compute_magnitude()
            );
            draw_text(
                &distance_from_sun,
                screen_width() - 300.0,
            ...
            let potential_energy = if let Some(sun_pos) = sun_position {
                format!(
                    ...
                    -crate::physics::G * body.mass * bodies.iter().find(|b| b.name == "Sun").unwrap().mass / body.position.subtract(sun_pos).compute_magnitude()
                )
            } else {
